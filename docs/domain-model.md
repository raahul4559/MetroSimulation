...
| **Signal** | `id` (`"SIG-{trackId}"`), `trackId` (where it stands), `protectedSectionId` (what it protects — always equal to `trackId` in this one-block-per-track model), `blockState` (`BlockState`: `FREE`/`RESERVED`/`OCCUPIED`), `aspect` (`SignalAspect`: `GREEN`/`YELLOW`/`RED`, one-to-one with `blockState`), `controllingTrainId` (null when `FREE`). One per track, computed fresh every tick by `TrainMovementTickHandler` from which train (if any) currently holds that block — never persisted or independently mutated, exactly like `TrainState`. See `docs/architecture.md`'s "Signaling and block sections". |
